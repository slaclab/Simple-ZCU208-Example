...
``Simple-ZCU208-Example`` firmware on the Xilinx ZCU208 evaluation board
(FPGA part ``XCZU48DR-FSVG1517-2-E``, firmware version ``v3.0.0.0`` /
...
.. admonition:: Reference toolchain
   ...
   - **Target FPGA part:** ``XCZU48DR-FSVG1517-2-E``
...
.. code-block:: bash
   ...
   source firmware/vivado_setup.sh
   cd firmware/targets/SimpleZcu208Example/
   make
...
``firmware/targets/SimpleZcu208Example/images/`` using the schema
``SimpleZcu208Example-0x03000000-<YYYYMMDDHHMMSS>-<user>-<git-short-SHA>``:

.. code-block:: text

   firmware/targets/SimpleZcu208Example/images/SimpleZcu208Example-0x03000000-<YYYYMMDDHHMMSS>-<user>-<git-short-SHA>.bit   (~33 MiB)
   firmware/targets/SimpleZcu208Example/images/SimpleZcu208Example-0x03000000-<YYYYMMDDHHMMSS>-<user>-<git-short-SHA>.xsa   (~3 MiB)
...
``image.ub``, ``boot.scr``, ``system.bit``) that run on the ZCU208 Processing
...
.. code-block:: bash
   ...
   cd firmware/targets/SimpleZcu208Example/
   ./BuildYoctoProject.sh -f images/SimpleZcu208Example-0x03000000-<YYYYMMDDHHMMSS>-<user>-<git-short-SHA>.xsa
...
``firmware/build/YoctoProjects/SimpleZcu208Example/linux/`` and a packaged
tarball is also produced at
``firmware/targets/SimpleZcu208Example/images/<full-name>.linux.tar.gz``.
...
copy live under
``firmware/build/YoctoProjects/SimpleZcu208Example/linux/``
(``BOOT.BIN``, ``image.ub``, ``boot.scr``, ``system.bit``).

Boot
----

1. Power down the ZCU208 board.
2. Confirm ``Mode SW2 [4:1] = 1110`` (switch OFF = 1 = High; ON = 0 = Low) for SD boot.
