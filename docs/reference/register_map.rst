...
:repo:`firmware/targets/SimpleZcu208Example/hdl/SimpleZcu208Example.vhd`
...
.. list-table::
   ...
   * - Index constant
     ...
     - Notes
   * - ``HW_INDEX_C = 0``
     - ``Hardware`` (ZCU208 board hardware)
     - Board hardware control (LMK, I2C). Sourced from
...
.. list-table::
   ...
   * - ``RING_INDEX_C = 0``
     - ``AppRingBuffer``
     - ADC/DAC capture ring buffers; DMA inbound path (8 ADC + 8 DAC channels).
   * - ``DAC_SIG_INDEX_C = 1``
     - ``SigGen`` (``DacSigGen``)
     - RAM-based DAC waveform generator (8 channels).
